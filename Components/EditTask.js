...
export default class EditTask extends Component {
    ...
    _setState = () => {
        ...
        db.transaction((tx) => {
            tx.executeSql('SELECT * FROM SMSTask WHERE id = ?',
            [id], (tx, rs) => {
                let row = rs.rows.item(0);
                this.setState({
                message: row.msg, title: row.title, contact: row.contact, date: row.time, time: row.time, contactName: [], 
                contactPhone: [], mode: row.mode, expanded: true});
                console.log(row.date);
                if(row.repeat != null) {
                    let repeat = row.repeat.split("; ");
                    this.setState({
                        repeat2: repeat.indexOf("2") == -1 ? false : true,
                        repeat3: repeat.indexOf("3") == -1 ? false : true,
                        repeat4: repeat.indexOf("4") == -1 ? false : true,
                        repeat5: repeat.indexOf("5") == -1 ? false : true,
                        repeat6: repeat.indexOf("6") == -1 ? false : true,
                        repeat7: repeat.indexOf("7") == -1 ? false : true,
                        repeat8: repeat.indexOf("1") == -1 ? false : true,
                    })
                }
            });
    ...
    _handleSubmit = async () => {
        ...
        if (this.state.mode == "datetime")
        {
            ...
            db.transaction((tx) => {
            tx.executeSql('UPDATE SMSTask SET title = ?, contact = ?, msg = ?, mode = ?, time = ? WHERE id = ?',
            [this.state.title, this.state.contact, this.state.message, this.state.mode, this.state.date, id], (tx, rs) => {
                SMS.cancel(id);
                SMS.schedule(this.state.contact, this.state.message, Number(time.split(":")[1]), Number(time.split(":")[0]),
                Number(date.split("-")[2]), Number(date.split("-")[1]), Number(date.split("-")[0]), id);
            ...
            db.transaction((tx) => {
            tx.executeSql('UPDATE SMSTask SET title = ?, contact = ?, msg = ?, mode = ?, time = ?, repeat = ? WHERE id = ?',
            [this.state.title, this.state.contact, this.state.message, this.state.mode, todayFormat + ' / ' + this.state.time, repeat, id], (tx, rs) => {
                SMS.cancel(id);
                SMS.cancel(id + 9999);
                SMS.scheduleRepeat(this.state.contact, this.state.message, Number(time[1]), Number(time[0]), repeat, id);
                this.props.navigation.navigate('List');
            });
            }, (tx, err) => console.log(err));
        }
